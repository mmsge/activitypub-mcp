...
| [0050](0050-the-probe-never-reached-a-video.md) | The probe never reached a video, and stage 1 kept asking answered questions — an EU IP is consent-walled, which cannot reproduce from anywhere that is not; and a queue ordered by fewest attempts re-asks the same head of the list forever once a success resets the counter |
| [0051](0051-a-race-side-is-an-entity-and-a-race-is-a-file.md) | A race side is an entity, and a race is a file — artist, album or track, matched exactly, with album and track sides taking a LIST because Last.fm files a single under its own album; several races at once, each with its own topic and ladder, defined in races.json rather than two environment variables |
| [0052](0052-the-state-keeps-its-columns-and-gains-a-key.md) | The race state keeps its columns and gains a key — keyed on race_id rather than the artist pair, with every dedupe column and the endgame timestamp latch intact; and the crossover of a race added after it turned is reconstructed from the archive, not read off the latest play |
| [0053](0053-the-import-wakes-the-profile-it-feeds.md) | The import wakes the profile it feeds, and only when something changed — bartenderen's four-hour cap is a backstop, not a mechanism, and nothing in the stored data predicts a leg entered mid-journey; fire on inserted+updated only, never throw, never retry, and log a drifted secret loudly |
| [0054](0054-an-export-is-evidence-of-absence-only-inside-its-own-range.md) | An export is evidence of absence only inside its own range — a viaduct CSV carries no tombstones, so a leg deleted upstream survived forever beside its correction; an import now reports what the file no longer contains, prunes only between the file's own earliest and latest departure, refuses a plan too large for that range, and deletes nothing until it is confirmed |
| [0055](0055-a-second-consumer-splits-transport-from-policy.md) | A second consumer splits the transport from the policy — msge.no proved 0053 right about the domain logic (its own module, config pair and calls; still no fan-out) and wrong about the transport, so never-throw/never-retry/log-loudly moved into one shared postWebhook and nothing else did; a topic names the upstream event, the ingest debounce needs a ceiling or a long backfill sends nothing, and scrobbles are deliberately not notified |
| [0056](0056-two-counters-meet-and-the-instant-is-the-key.md) | Two counters meet, and the instant is the key — a scrobble steps by one so it can never skip zero, which makes every crossover a kilometre lump; the crossing is keyed on when it happened rather than on a trip uuid a re-import replaces; a leg counts from when it DEPARTED, not from a frozen `Planned`; and the watermark is sound for scrobbles and never for legs |
| [0057](0057-store-the-shape-of-a-conversation-not-its-words.md) | Store the shape of a conversation, not its words — walk each of his own toots' threads once per root, keep ids, links, depths and handles, and let CHECK constraints make reply text unstorable rather than merely absent |
| [0058](0058-an-account-there-is-not-a-server-of-ours.md) | An account there is not a server of ours — the engagement sampler's auto-watchlist polled every followed origin hourly, including instances somebody else runs; `ENGAGEMENT_SAMPLE_ORIGINS` bounds the unattended jobs (never the tool), empty means poll nothing rather than everything, and a REST 401 falls through to the AP object instead of being read as a private post |
| [0059](0059-a-day-is-the-day-you-lived.md) | A day is the day you lived — `get_scrobble_timeline` buckets the archive on `played_at AT TIME ZONE $tz`, because bucketing on the UTC date misfiles every summer evening after 22:00 local by a day and no query fails while it does; day cannot be a `group_by` value on the stats tool because `group_by` there selects the entity ranked, the zone is a bound parameter validated before the query, the bucket enumeration deliberately carries no timezone so the rule has one home, a too-wide range is clamped to the archive rather than refused, and MCP/REST differ in exactly two defaults |
