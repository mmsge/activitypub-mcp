...
export const endpoints: RestEndpoint[] = [
  ...
  },
  {
    ...
    description: 'Get BookWyrm reading events for an actor, derived from stored note posts with a normalized event_type field: started_reading, finished_reading, stopped_reading, review, rating, comment, quotation, note, shelved. Events carry derived signal dates (started_date/finished_date), the book\'s overall window (book_started_date/book_finished_date), inline rating, review_title, quote (for quotations), and progress/progress_mode when present. Defaults to newest-first; set sort_order=asc with limit=1 for the earliest event, and follow next_cursor for deep traversal.',
  ...
  },
  {
    ...
    description: "Aggregate metrics over the YouTube watch history: total watches, distinct videos and channels, first/last watch, and a breakdown by channel, year, month, day, weekday, hour_of_day, account or video. Accepts the same filters as /youtube-watches, so totals and first/last reflect only matching rows. Every response carries its own caveats and they are not optional garnish: watch_time_estimates returns THREE upper bounds (raw, capped at 20 min, Shorts-excluded) because this data contains NO watch duration at all, with duration_coverage_pct saying what share of rows even had a length; shorts_split keeps unknown_duration as its own bucket rather than folding it into either side; unresolved_watches and watches_without_channel report how much of the result has no title or channel; and with group_by=channel, excluded_from_ranking states how many watches the ranking could not include. distinct_channels counts channel IDS, with distinct_channel_names beside it, since a renamed channel has one id and two names. year/month/day/weekday/hour_of_day come back in calendar order on Europe/Oslo local time; group_count shows whether `limit` truncated the breakdown. `day` spans ~2,500 buckets archive-wide against a limit of 500, so scope it with year or from/to; `weekday` is ISO numbering, 1=Monday through 7=Sunday. Every `top` bucket carries distinct_videos, first_watch and last_watch beside its counts, so a per-channel lifecycle is one call rather than one call per channel.",
  ...
  },
  {
    path: '/books',
    name: 'get_books',
    description: "Browse all cached BookWyrm book metadata as a paginated catalogue. Returns compact rows (book_url, title, subtitle, author, series, pages, physical_format, isbn13/isbn10, pub_year, language, publisher, cover_url, subjects, shelf, shelves, fetched_at) — call get_book_details for the full record (description, provenance) of one book. Filter by title (partial match), author (partial match), format, language, series (partial match), subject (partial match against any subject/genre), or `shelf`. NOTE ON `shelf`: this endpoint is a per-edition BIBLIOGRAPHIC cache, not a shelf — unfiltered it returns every book ever enriched, including ones still being read and ones started and put down. Pass shelf=read for finished books only; the other values are reading, to-read, stopped-reading, and `unknown` for cached editions with no live shelf membership. A shelf filter errors rather than returning an empty page when shelf membership has not been synced yet, because an empty page is indistinguishable from 'nothing is on that shelf'. Most-recently-enriched first by default (sort_order='asc' for oldest first). Each response carries `total` (matching books across all pages) and a `next_cursor` token; pass it back as `cursor` for deep traversal, or use the legacy offset `page`.",
  ...
  },
  {
    path: '/watched',
    name: 'get_watched',
    description: "Browse the cached NeoDB catalogue (ALL categories) as a paginated table — every item behind this server's stored NeoDB marks, enriched from the linked catalog item. Filter by `category`: tv, movie, book, music, game, podcast, performance. Common fields on every row (item_url, category, item_type, title/display_title/orig_title, year, cover_url, description, genre, language, area, rating, external_resources, fetched_at); film/TV columns (season_number, episode_count, imdb/imdb_url, tmdb_url, director, actors); and a per-category `details` object (book author/isbn/pages/publisher + bookwyrm_book_url; music artist/release_date/track_count/barcode; game developer/publisher/platform; podcast host/feed_url; performance playwright/director/troupe/venue/opening_date). Every row also carries mark_comments (the note(s) the mark(s) carried, verbatim and unparsed, newest first), the mark's shelf state (status: wishlist | progress | complete | dropped, from the NEWEST live mark, plus status_raw verbatim and statuses newest-first), and the shelf date: watched_at (when it was actually watched/read/played/listened to — NOT the post timestamp; the newest date when an item was marked more than once) plus watched_dates (every distinct date, newest first). Filter by title/category/item_type/genre/mark_comment (partial) or exact imdb id; narrow by shelf state with `status` (positive — REQUIRES a tracked mark, so items enriched before the mark store existed are excluded) or `exclude_status` (negative — drops only what is positively known, keeping untracked items; exclude_status=dropped,progress is the honest way to ask for only-what-I-actually-finished); narrow to a period with watched_from/watched_to (YYYY-MM-DD, UTC, both ends inclusive) or watched_year=2016 sugar; watched_date_unknown=true lists titles seen with the date explicitly unknown (dated 2000-01-01 on minreol; their watched_at is null and they belong to no watch year), =false drops them; include_unenriched=true also returns pending/failed rows. sort_by=watched_at orders by the shelf date (sort_order=asc for oldest first) — the default sort_by=fetched_at is enrichment time, which for a backfilled import is just the order the import ran in. Carries `total` and `next_cursor` (or legacy offset `page`). Use /catalogue-details for one item's full record + provenance.",
    schema: getWatchedSchema,
    handler: getWatched,
    numbers: ['limit', 'page', 'watched_year'],
    booleans: ['include_unenriched', 'include_hidden', 'watched_date_unknown'],
    arrays: ['exclude_status'],
  },
  {
    path: '/catalogue-details',
    name: 'get_catalogue_details',
    description: "Get one NeoDB catalogue item's full cached record, resolved by item_url (exact) or a partial title (pass category to disambiguate). Returns the common fields, mark_comments (the note(s) the mark(s) carried, verbatim), watched_at/watched_dates (the shelf date(s) — when it was watched/read/played, not when the mark was posted) and watched_date_unknown (true when it was marked as seen with the date explicitly unknown, so watched_at is null on purpose), film/TV columns, and the category-specific `details` object, plus bookwyrm_book_url (book↔BookWyrm dedup link), source_map (per-field 'neodb'|'bookwyrm' provenance), and enrichment status (fetched_at, fetch_error, fetch_attempts, last_attempt_at). The catalogue sibling of /book-details.",
