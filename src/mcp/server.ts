...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    'get_youtube_stats',
    "Aggregate metrics over the YouTube watch history: total watches, distinct videos, distinct channels, the first and last watch, and a breakdown by channel, year, month, day, weekday, hour_of_day, account or video. Accepts the same filters as get_youtube_watches — when filtered, total_watches and first/last_watched_at_local reflect ONLY matching rows, so 'when did I first watch this channel' is a single call with channel set. EVERY RESPONSE CARRIES ITS OWN CAVEATS, AND YOU MUST PASS THEM ON. watch_time_estimates returns THREE figures, never one, because THIS DATA CONTAINS NO WATCH DURATION AT ALL — only that a video was opened. raw_hours sums full video lengths and is a gross overestimate (an eight-hour livestream left open for two minutes counts as eight hours). capped_20min_hours caps each row at 20 minutes and is the least-bad figure for anything resembling 'time spent'. excluding_shorts_hours counts everything NOT believed to be a Short. All three are UPPER BOUNDS. duration_coverage_pct says what share of the matching rows had a duration at all (about 89% archive-wide) — the rest contribute zero to every estimate, so the true figure is not merely lower, it is unmeasurable. shorts_split IS THE ONE TO READ BEFORE QUOTING ANY SHORTS NUMBER, because the Shorts flag is DERIVED BY THIS SERVER and not supplied by YouTube. Its headline shorts / long_form / unknown are the resolved split, and beneath them known_short and known_long_form are the rows actually decided, while guessed_short and guessed_long_form are the rows still falling back to the flat 'under 180 seconds' rule — which over-counts Shorts, since they did not exist before September 2020 and were capped at 60s until 15 October 2024. by_method breaks the known ones down into duration_rule / api_metadata / probe, and only 'probe' can ever CONFIRM a Short; the other two can only rule one out. unclassifiable counts deleted and private videos, which nothing will ever decide. Quote 'X Shorts' only alongside how many of them are known versus guessed, and never fold unknowns into either side. unresolved_watches and watches_without_channel report how much of the result has no title or channel — about 11.4% archive-wide. WITH group_by='channel', excluded_from_ranking states exactly how many watches the ranking could not include for having no channel; quote it alongside any top-channels answer rather than letting the ranking imply full coverage. distinct_channels counts CHANNEL IDS, with distinct_channel_names beside it: a channel that renamed has one id and two names, so the two numbers answer different questions and neither is a typo. group_by='year'/'month'/'day'/'weekday'/'hour_of_day' return CALENDAR ORDER ascending, bucketed on Europe/Oslo local time; the others rank by watch count descending. 'day' spans ~2,500 buckets archive-wide against a limit of 500, so scope it with year or from/to rather than asking for the whole series; 'weekday' is ISO numbering, 1=Monday through 7=Sunday, labelled with the short day name. group_count says how many buckets exist, so raise `limit` (max 500) rather than assuming `top` is complete — month alone spans ~190 buckets. EVERY top BUCKET also carries distinct_videos, first_watch and last_watch beside its counts, so 'when did this channel start and stop' is answerable for a whole ranking in ONE call rather than one call per channel; first_watch/last_watch are Europe/Oslo local wall clock like every other time here. ONE MORE TRAP: 2025 carries 53,360 watches against 2024's 866, a 62x discontinuity that Shorts alone do not explain and that is far more likely a change in what was being recorded than a change in viewing. Do not present a year-on-year trend across that boundary as a smooth curve — exclude the discontinuity or annotate it explicitly.",
